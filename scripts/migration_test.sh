...
MYSQL="mysql -u root -paBc@123456 -h 127.0.0.1"
SRC=shopxo_mig_test_src
DST=shopxo_mig_test_dst

echo "=== 1. 创建测试库 ==="
$MYSQL -e "DROP DATABASE IF EXISTS $SRC; DROP DATABASE IF EXISTS $DST;"
$MYSQL -e "CREATE DATABASE $SRC CHARACTER SET utf8mb4; CREATE DATABASE $DST CHARACTER SET utf8mb4;"
...
sed -i '' "s/dbname: goshop/dbname: $DST/" config.yaml
GOSHOP_E2E=1 ./bin/goshop &
PID=$!; sleep 4; kill $PID 2>/dev/null; sleep 1

echo "=== 4. 执行迁移 SQL ==="
# 清空 seed 数据（GoShop 首次启动会插入示例数据）
$MYSQL $DST -e "SET FOREIGN_KEY_CHECKS=0; TRUNCATE users; TRUNCATE categories; TRUNCATE goods; TRUNCATE goods_skus; TRUNCATE orders; TRUNCATE order_items; TRUNCATE addresses; TRUNCATE brands; TRUNCATE articles; TRUNCATE admins; SET FOREIGN_KEY_CHECKS=1;" 2>/dev/null
sed "s/shopxo_mig_src/$SRC/g; s/goshop_mig_dst/$DST/g" scripts/run_shopxo_migration_local_test.sql | $MYSQL 2>&1

echo "=== 5. 验证数据 ==="
FAIL=0
check() {
  local desc="$1" sql="$2" expect="$3"
  result=$($MYSQL -N -e "$sql" $DST 2>/dev/null | tr -d '[:space:]')
...
echo ""
echo "=== 6. API 验证 ==="
$MYSQL $DST -e "UPDATE admins SET password='\$2a\$10\$Q7LicCzsoEGtbWhumeGrQOzrK5F2/dO2bbjN5De2jEewY8tCxCPgy' WHERE id=1;"
sed -i '' "s/dbname: goshop/dbname: $DST/" config.yaml
GOSHOP_E2E=1 ./bin/goshop &
PID=$!; sleep 3
...
if echo "$LOGIN" | grep -q '"code":0'; then
  ...
  curl -s http://127.0.0.1:8080/api/categories | grep -q '手机数码' && echo "  ✅ 分类API" || { echo "  ❌ 分类API"; FAIL=1; }
else
...
echo ""
echo "=== 7. 清理 ==="
cp config.yaml.bak config.yaml; rm -f config.yaml.bak
$MYSQL -e "DROP DATABASE IF EXISTS $SRC; DROP DATABASE IF EXISTS $DST;"
kill $(lsof -ti :8080) 2>/dev/null || true
sleep 1
