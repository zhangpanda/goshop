-- 嵌入资源：由 cmd/shopxo-import 将 __SHOPXO_DB__ / __GOSHOP_DB__ 及表前缀替换为实际值。ShopXO v6.x 数据导入（与 docs/migration-from-shopxo.md 一致）。

SET NAMES utf8mb4;
SET FOREIGN_KEY_CHECKS = 0;

-- ========== 1.1 用户 ==========
INSERT INTO __GOSHOP_DB__.users (id, username, password, nickname, phone, avatar, points, locking_integral, status, created_at, updated_at)
SELECT
  id,
  username,
  '',
...
FROM __SHOPXO_DB__.sxo_user
WHERE IFNULL(is_delete_time, 0) = 0;

-- ========== 1.2 分类 ==========
INSERT INTO __GOSHOP_DB__.categories (id, parent_id, name, icon, sort, status, created_at, updated_at)
SELECT
  ...
  FROM_UNIXTIME(IF(add_time = 0, UNIX_TIMESTAMP(), add_time)),
  FROM_UNIXTIME(IF(IFNULL(upd_time, 0) = 0, add_time, upd_time))
FROM __SHOPXO_DB__.sxo_goods_category;

-- ========== 1.3 商品 ==========
INSERT INTO __GOSHOP_DB__.goods (
...
SELECT
  ...
  COALESCE(
    (SELECT CAST(CONCAT('[', GROUP_CONCAT(JSON_QUOTE(p.images) ORDER BY IFNULL(p.sort, 0), p.id SEPARATOR ','), ']') AS JSON)
     FROM __SHOPXO_DB__.sxo_goods_photo p
...
FROM __SHOPXO_DB__.sxo_goods g
LEFT JOIN (
  SELECT goods_id, MIN(category_id) AS category_id
  FROM __SHOPXO_DB__.sxo_goods_category_join
  GROUP BY goods_id
) cj ON cj.goods_id = g.id
WHERE IFNULL(g.is_delete_time, 0) = 0;

-- ========== 1.4 SKU（spec_base） ==========
INSERT INTO __GOSHOP_DB__.goods_skus (
...
SELECT
  ...
  FROM_UNIXTIME(IF(b.add_time = 0, UNIX_TIMESTAMP(), b.add_time))
FROM __SHOPXO_DB__.sxo_goods_spec_base b
LEFT JOIN __SHOPXO_DB__.sxo_goods_spec_value v
...
WHERE IFNULL(g.is_delete_time, 0) = 0
  AND NOT EXISTS (
    SELECT 1 FROM __SHOPXO_DB__.sxo_goods_spec_base b WHERE b.goods_id = g.id
  );

-- ========== 1.5 订单 ==========
INSERT INTO __GOSHOP_DB__.orders (
...
SELECT
  ...
  IFNULL(
    ...
    FROM __SHOPXO_DB__.sxo_order_address oa
...
FROM __SHOPXO_DB__.sxo_order o
WHERE IFNULL(o.is_delete_time, 0) = 0;

-- ========== 1.6 订单明细 ==========
INSERT INTO __GOSHOP_DB__.order_items (
...
SELECT
  ...
  d.goods_id,
  COALESCE(
    (SELECT b1.id FROM __SHOPXO_DB__.sxo_goods_spec_base b1
     WHERE b1.goods_id = d.goods_id AND b1.coding = d.spec_coding AND IFNULL(d.spec_coding, '') <> ''
     LIMIT 1),
    (SELECT MIN(b2.id) FROM __SHOPXO_DB__.sxo_goods_spec_base b2 WHERE b2.goods_id = d.goods_id),
...
FROM __SHOPXO_DB__.sxo_order_detail d;

-- ========== 1.7 收货地址（v6.8：省市区为 region id，JOIN sxo_region） ==========
INSERT INTO __GOSHOP_DB__.addresses (id, user_id, name, phone, province, city, district, detail, is_default, created_at, updated_at)
SELECT
  ua.id,
  ua.user_id,
  ua.name,
...
FROM __SHOPXO_DB__.sxo_user_address ua
LEFT JOIN __SHOPXO_DB__.sxo_region rp ON rp.id = ua.province
LEFT JOIN __SHOPXO_DB__.sxo_region rc ON rc.id = ua.city
LEFT JOIN __SHOPXO_DB__.sxo_region rd ON rd.id = ua.county;

-- ========== 1.8 品牌 / 文章 / 管理员 ==========
INSERT INTO __GOSHOP_DB__.brands (id, name, logo, sort, status, created_at, updated_at)
SELECT
  id, name, IFNULL(logo, ''), IFNULL(sort, 0), IFNULL(is_enable, 1),
  FROM_UNIXTIME(IF(add_time = 0, UNIX_TIMESTAMP(), add_time)),
  FROM_UNIXTIME(IF(IFNULL(upd_time, 0) = 0, add_time, upd_time))
FROM __SHOPXO_DB__.sxo_brand;

INSERT INTO __GOSHOP_DB__.articles (
...
SELECT
  ...
  FROM_UNIXTIME(IF(add_time = 0, UNIX_TIMESTAMP(), add_time)),
  FROM_UNIXTIME(IF(IFNULL(upd_time, 0) = 0, add_time, upd_time))
FROM __SHOPXO_DB__.sxo_article;

INSERT INTO __GOSHOP_DB__.admins (id, username, password, nickname, role_id, status, created_at, updated_at)
SELECT
  id,
  username,
  '',
