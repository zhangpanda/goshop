...
func GoodsListHandle(list []model.Goods) []model.Goods {
	for i := range list {
		if list[i].SKUs == nil {
			app.Must().DB.Where("goods_id = ?", list[i].ID).Find(&list[i].SKUs)
		}
	}
	return list
}
...
func GoodsCategoryDelete(id uint) error {
	...
	return app.Must().DB.Delete(&model.Category{}, id).Error
}
func GoodsCategoryItemsIds(ids []uint, deep int) []uint {
	...
	for d := 0; d < deep; d++ {
		var childIDs []uint
		app.Must().DB.Model(&model.Category{}).Where("parent_id IN ? AND status = 1", result).Pluck("id", &childIDs)
		if len(childIDs) == 0 {
			break
		}
...
func GoodsCategoryName(id uint) string {
	...
	app.Must().DB.Select("name").First(&c, id)
	return c.Name
}
func GoodsCategoryNames(ids []uint) map[uint]string {
	var cats []model.Category
	app.Must().DB.Where("id IN ?", ids).Select("id, name").Find(&cats)
...
func GoodsCategoryParentIds(id uint) []uint {
	...
	for id > 0 {
		var c model.Category
		app.Must().DB.Select("id, parent_id").First(&c, id)
		if c.ID == 0 {
			break
		}
...
func GoodsCategoryAll() []model.Category {
	var cats []model.Category
	app.Must().DB.Where("status = 1").Order("sort DESC, id ASC").Find(&cats)
...
func GoodsCommentsDelete(id uint) error { return app.Must().DB.Delete(&model.Review{}, id).Error }

/**
 * GoodsCommentsDeleteForUser 仅允许删除当前用户的评价。
 */
func GoodsCommentsDeleteForUser(id, userID uint) error {
	res := app.Must().DB.Where("id = ? AND user_id = ?", id, userID).Delete(&model.Review{})
	if res.Error != nil {
		return res.Error
	}
...
func GoodsFirstSeveralComments(goodsID uint, n int) []model.Review {
	...
	app.Must().DB.Where("goods_id = ?", goodsID).Preload("User").Order("id DESC").Limit(n).Find(&list)
	return list
}
...
func GoodsFavorDelete(ids []uint, userID uint) error {
	return app.Must().DB.Where("id IN ? AND user_id = ?", ids, userID).Delete(&model.Favorite{}).Error
}

// ==================== GoodsBrowse ====================
func GoodsBrowseDelete(ids []uint, userID uint) error {
	return app.Must().DB.Where("id IN ? AND user_id = ?", ids, userID).Delete(&model.BrowseHistory{}).Error
}
...
func GoodsCartStock(cartID uint) (int, error) {
	var cart model.Cart
	app.Must().DB.Preload("SKU").First(&cart, cartID)
	if cart.SKU == nil {
...
func BrandDeleteFull(id uint) error { return app.Must().DB.Delete(&model.Brand{}, id).Error }
func BrandTotal() int64             { return totalCount(&model.Brand{}, "status = ?", 1) }

// ==================== BrandCategory ====================
func BrandCategoryDelete(id uint) error {
	return app.Must().DB.Delete(&model.BrandCategory{}, id).Error
}
...
func ArticleDeleteFull(id uint) error { return app.Must().DB.Delete(&model.Article{}, id).Error }
...
func ArticleCategoryDelete(id uint) error {
	var count int64
	app.Must().DB.Model(&model.Article{}).Where("category_id = ?", id).Count(&count)
	if count > 0 {
		return errHasChildren
	}
	return app.Must().DB.Delete(&model.ArticleCategory{}, id).Error
}
...
func ExpressDelete(id uint) error { return app.Must().DB.Delete(&model.Express{}, id).Error }

// ==================== Slide ====================
func SlideStatusUpdate(id uint, status int8) error {
	return statusUpdate("slides", id, "status", status)
}
func SlideDelete(id uint) error { return app.Must().DB.Delete(&model.Slide{}, id).Error }

// ==================== Navigation ====================
func NavStatusUpdate(id uint, status int8) error {
	return statusUpdate("navigations", id, "status", status)
}
func NavDelete(id uint) error { return app.Must().DB.Delete(&model.Navigation{}, id).Error }
func NavSave(n *model.Navigation) error {
	if n.ID > 0 {
		return app.Must().DB.Save(n).Error
	}
	return app.Must().DB.Create(n).Error
}
func BottomNavigationData() []model.Navigation {
	var l []model.Navigation
	app.Must().DB.Where("type = 'footer' AND status = 1").Order("sort DESC").Find(&l)
	return l
}
func LevelOneNav() []model.Navigation {
	var l []model.Navigation
	app.Must().DB.Where("type = 'header' AND status = 1").Order("sort DESC").Find(&l)
	return l
}
func UserCenterLeftList() []model.Navigation {
	var l []model.Navigation
	app.Must().DB.Where("type = 'user_center' AND status = 1").Order("sort DESC").Find(&l)
	return l
}

// ==================== Link ====================
func LinkStatusUpdate(id uint, status int8) error { return statusUpdate("links", id, "status", status) }
func LinkDelete(id uint) error                    { return app.Must().DB.Delete(&model.Link{}, id).Error }
...
func ScreeningPriceDelete(id uint) error {
	return app.Must().DB.Delete(&model.ScreeningPrice{}, id).Error
}

// ==================== Region ====================
func RegionSave(r *model.Region) error {
	if r.ID > 0 {
		return app.Must().DB.Save(r).Error
	}
	return app.Must().DB.Create(r).Error
}
func RegionDelete(id uint) error {
	var count int64
	app.Must().DB.Model(&model.Region{}).Where("parent_id = ?", id).Count(&count)
	if count > 0 {
		return errHasChildren
	}
	return app.Must().DB.Delete(&model.Region{}, id).Error
}
...
func RegionAll() []model.Region {
	...
	app.Must().DB.Order("sort, id").Find(&list)
	return list
}
func RegionCodeData(id uint) *model.Region {
	var r model.Region
	app.Must().DB.First(&r, id)
	return &r
}
func RegionName(id uint) string {
	var r model.Region
	app.Must().DB.Select("name").First(&r, id)
	return r.Name
}
func RegionNodeSon(pid uint) []model.Region {
	var list []model.Region
	app.Must().DB.Where("parent_id = ?", pid).Order("sort, id").Find(&list)
	return list
}
func RegionItemsIds(pid uint) []uint {
	var ids []uint
	app.Must().DB.Model(&model.Region{}).Where("parent_id = ?", pid).Pluck("id", &ids)
	return ids
}
...
func CustomViewDelete(id uint) error { return app.Must().DB.Delete(&model.CustomView{}, id).Error }

// ==================== Design ====================
func DesignStatusUpdate(id uint, status int8) error {
	return statusUpdate("designs", id, "status", status)
}
func DesignDelete(id uint) error { return app.Must().DB.Delete(&model.Design{}, id).Error }
func DesignAccessCountInc(id uint) {
	app.Must().DB.Model(&model.Design{}).Where("id = ?", id).Update("status", gorm.Expr("COALESCE(status,0)"))
}

// ==================== Diy ====================
func DiyStatusUpdate(id uint, status int8) error { return statusUpdate("diys", id, "status", status) }
func DiyAccessCountInc(id uint) {
	app.Must().DB.Model(&model.Diy{}).Where("id = ?", id).Update("access_count", gorm.Expr("access_count + 1"))
}
...
func AppHomeNavDelete(id uint) error { return app.Must().DB.Delete(&model.AppHomeNav{}, id).Error }

// ==================== AppCenterNav ====================
func AppCenterNavStatusUpdate(id uint, status int8) error {
	return statusUpdate("app_center_navs", id, "status", status)
}
func AppCenterNavDelete(id uint) error { return app.Must().DB.Delete(&model.AppCenterNav{}, id).Error }

// ==================== ShortcutMenu ====================
func ShortcutMenuDelete(id uint) error { return app.Must().DB.Delete(&model.ShortcutMenu{}, id).Error }

// ==================== QuickNav ====================
func QuickNavStatusUpdate(id uint, status int8) error {
	return statusUpdate("quick_navs", id, "status", status)
}
func QuickNavDelete(id uint) error { return app.Must().DB.Delete(&model.QuickNav{}, id).Error }

// ==================== Attachment ====================
func AttachmentDelete(id uint) error { return app.Must().DB.Delete(&model.Attachment{}, id).Error }
func AttachmentSave(a *model.Attachment) error {
	if a.ID > 0 {
		return app.Must().DB.Save(a).Error
	}
	return app.Must().DB.Create(a).Error
}
...
func AttachmentCategoryDelete(id uint) error {
	return app.Must().DB.Delete(&model.AttachmentCategory{}, id).Error
}

// ==================== ErrorLog ====================
func ErrorLogDelete(ids []uint) error {
	return app.Must().DB.Where("id IN ?", ids).Delete(&model.ErrorLog{}).Error
}
func ErrorLogAllDelete() error { return app.Must().DB.Where("1=1").Delete(&model.ErrorLog{}).Error }

// ==================== PluginsCategory ====================
func PluginsCategorySave(name string, sort int) error {
	return app.Must().DB.Create(&model.PluginCategory{Name: name, Sort: sort}).Error
}
func PluginsCategoryDelete(id uint) error {
	return app.Must().DB.Delete(&model.PluginCategory{}, id).Error
}
...
func AdminDetail(id uint) (*model.Admin, error) {
	var a model.Admin
	if err := app.Must().DB.Preload("Role").First(&a, id).Error; err != nil {
		return nil, err
	}
	return &a, nil
}
func AdminSave(id uint, nickname string, roleID uint) error {
	return app.Must().DB.Model(&model.Admin{}).Where("id = ?", id).Updates(map[string]interface{}{"nickname": nickname, "role_id": roleID}).Error
}
func AdminDelete(id uint) error { return app.Must().DB.Delete(&model.Admin{}, id).Error }
...
func MessageDelete(ids []uint, userID uint) error {
	return app.Must().DB.Where("id IN ? AND user_id = ?", ids, userID).Delete(&model.Message{}).Error
}
