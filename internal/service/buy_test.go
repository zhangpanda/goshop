...
func TestClearCache(t *testing.T) {
	setupTestCache()
	app.Must().Cache.Set(context.Background(), "test:a", "1", time.Minute)
	app.Must().Cache.Set(context.Background(), "test:b", "2", time.Minute)
	app.Must().Cache.Set(context.Background(), "other:c", "3", time.Minute)
	if err := ClearCache("test:"); err != nil {
		t.Fatal(err)
	}
	// test:* should be gone
	if _, err := app.Must().Cache.Get(context.Background(), "test:a"); err == nil {
		t.Error("test:a should be cleared")
	}
	// other:c should remain
	if v, err := app.Must().Cache.Get(context.Background(), "other:c"); err != nil || v != "3" {
...
func TestClearCache_All(t *testing.T) {
	setupTestCache()
	app.Must().Cache.Set(context.Background(), "x", "1", time.Minute)
	if err := ClearCache("all"); err != nil {
		t.Fatal(err)
	}
	n, _ := app.Must().Cache.DBSize(context.Background())
	if n != 0 {
