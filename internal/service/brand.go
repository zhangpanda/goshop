...
func CreateBrand(name, logo, desc string, sort int) (*model.Brand, error) {
	...
	return &b, app.Must().DB.Create(&b).Error
}

func GetBrandList() ([]model.Brand, error) {
	var list []model.Brand
	return list, app.Must().DB.Where("status = 1").Order("sort DESC, id DESC").Find(&list).Error
}

func UpdateBrand(id uint, name, logo, desc string, sort int) error {
	return app.Must().DB.Model(&model.Brand{}).Where("id = ?", id).Updates(map[string]interface{}{
		"name": name, "logo": logo, "desc": desc, "sort": sort,
	}).Error
}

func DeleteBrand(id uint) error { return app.Must().DB.Delete(&model.Brand{}, id).Error }
