...
func GetStatisticalExt() *StatisticalExt {
	...
	// 利润（简化：实付-退款）
	app.Must().DB.Model(&model.Order{}).Where("status IN ?", []int8{1, 2, 3}).Select("COALESCE(SUM(pay_amount),0)").Scan(&d.OrderProfitTotal)
	var refund int64
	app.Must().DB.Model(&model.RefundLog{}).Where("status = 1").Select("COALESCE(SUM(refund_price),0)").Scan(&refund)
	d.OrderProfitTotal -= refund
	// 支付方式统计
	app.Must().DB.Model(&model.PayLog{}).Where("status = 1").Select("client_type as payment_key, COUNT(*) as count, SUM(total_price) as amount").Group("client_type").Find(&d.PayTypeTotal)
	// 付费用户数
	app.Must().DB.Model(&model.Order{}).Where("status > 0").Distinct("user_id").Count(&d.BuyUserTotal)
	// 新用户
	app.Must().DB.Model(&model.User{}).Where("DATE(created_at) = ?", today).Count(&d.NewUserToday)
	app.Must().DB.Model(&model.User{}).Where("DATE(created_at) = ?", yesterday).Count(&d.NewUserYesterday)
	// 完成订单
	app.Must().DB.Model(&model.Order{}).Where("status = 3 AND DATE(completed_at) = ?", today).Count(&d.OrderCompleteToday)
	app.Must().DB.Model(&model.Order{}).Where("status = 3 AND DATE(completed_at) = ?", yesterday).Count(&d.OrderCompleteYesterday)
	return d
}
...
func UserAvatarUpload(userID uint, avatarURL string) error {
	return app.Must().DB.Model(&model.User{}).Where("id = ?", userID).Update("avatar", avatarURL).Error
}
...
func UserDelete(userID uint) error {
	return app.Must().DB.Delete(&model.User{}, userID).Error
}
...
func OrderDirectSuccess(orderID uint) error {
	now := time.Now()
	app.Must().DB.Model(&model.Order{}).Where("id = ?", orderID).Updates(map[string]interface{}{
...
func OrderExtractionData(orderID uint) *ExtractionInfo {
	var order model.Order
	app.Must().DB.Select("extraction_code, address").First(&order, orderID)
...
func OrderItemList(orderID uint) []model.OrderItem {
	...
	app.Must().DB.Where("order_id = ?", orderID).Find(&list)
	return list
}
...
func PayLogClose(payNo string) error {
	now := time.Now()
	return app.Must().DB.Model(&model.PayLog{}).Where("pay_no = ? AND status = 0", payNo).
...
func ConfigList() ([]model.Config, error) {
	...
	return list, app.Must().DB.Order("`group`, `key`").Find(&list).Error
}
...
func DiyApiCustomInit(diyID uint) (map[string]interface{}, error) {
	var diy model.Diy
	if err := app.Must().DB.First(&diy, diyID).Error; err != nil {
		return nil, err
	}
...
func AppMiniUserInfo(userID uint, platform string) (*model.UserPlatform, error) {
	...
	if err := app.Must().DB.Where("user_id = ? AND platform = ?", userID, platform).First(&p).Error; err != nil {
...
func AppMiniOnekeyUserMobileBind(userID uint, mobile string) error {
	return app.Must().DB.Model(&model.User{}).Where("id = ?", userID).Update("phone", mobile).Error
}

// FormInputApiList Form表单API列表
func FormInputApiList(page, pageSize int) ([]model.FormInput, int64, error) {
	var total int64
	app.Must().DB.Model(&model.FormInput{}).Where("status = 1").Count(&total)
	var list []model.FormInput
	err := app.Must().DB.Where("status = 1").Order("id DESC").Offset((page - 1) * pageSize).Limit(pageSize).Find(&list).Error
	return list, total, err
}
...
func FormInputApiDetail(id uint) (*model.FormInput, error) {
	...
	if err := app.Must().DB.First(&f, id).Error; err != nil {
		return nil, err
	}
	return &f, nil
}
...
func FormInputApiDelete(id, userID uint) error {
	return app.Must().DB.Where("id = ? AND user_id = ?", id, userID).Delete(&model.FormInputData{}).Error
}
