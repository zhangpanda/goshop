...
func ApplyDistributor(userID, parentID uint) (*model.Distributor, error) {
	...
	if global.DB.Where("user_id = ?", userID).First(&exists).Error == nil {
...
func GetDistributorByUser(userID uint) (*model.Distributor, error) {
	...
	if err := global.DB.Where("user_id = ?", userID).First(&d).Error; err != nil {
...
func GetDistributorList(page, pageSize int) (int64, []model.Distributor, error) {
	var total int64
	global.DB.Model(&model.Distributor{}).Count(&total)
...
func GetSubDistributors(userID uint) ([]model.Distributor, error) {
	var list []model.Distributor
	return list, global.DB.Where("parent_id = ?", userID).Find(&list).Error
}
...
func SettleCommission(orderID uint) {
	...
	if global.DB.First(&order, orderID).Error != nil || order.Status != model.OrderStatusCompleted {
		return
	}
	// 下单用户的上级
	var buyer model.Distributor
	if global.DB.Where("user_id = ?", order.UserID).First(&buyer).Error != nil {
		return
	}
	if buyer.ParentID == 0 {
		return
	...
	if global.DB.Where("user_id = ?", buyer.ParentID).First(&parent).Error == nil && parent.ParentID > 0 {
...
func addCommission(userID, orderID uint, amount int64, remark string) {
	var d model.Distributor
	if global.DB.Where("user_id = ? AND status = 1", userID).First(&d).Error != nil {
		return
	}
	global.DB.Model(&d).Updates(map[string]interface{}{
		"balance":          gorm.Expr("balance + ?", amount),
		"total_commission": gorm.Expr("total_commission + ?", amount),
		"order_count":      gorm.Expr("order_count + 1"),
	})
	global.DB.Create(&model.CommissionLog{
...
func RequestWithdraw(userID uint, amount int64, accountType, accountNo, accountName string) error {
	...
	var d model.Distributor
	if err := global.DB.Where("user_id = ? AND status = 1", userID).First(&d).Error; err != nil {
		return errors.New("非分销商")
	}
	// 原子扣减余额
	result := global.DB.Model(&d).Where("balance >= ?", amount).
		Update("balance", gorm.Expr("balance - ?", amount))
	if result.RowsAffected == 0 {
		return fmt.Errorf("余额不足(可提现%d分)", d.Balance)
	}
	global.DB.Create(&model.WithdrawRequest{
...
func GetWithdrawList(page, pageSize int, status *int8) (int64, []model.WithdrawRequest, error) {
	var total int64
	db := global.DB.Model(&model.WithdrawRequest{})
	if status != nil {
...
func AuditWithdraw(id uint, approve bool, reason string) error {
	...
	now := time.Now()
	if approve {
		global.DB.Model(&w).Updates(map[string]interface{}{"status": 1, "audit_at": &now})
	} else {
		// 退回余额
		global.DB.Model(&model.Distributor{}).Where("id = ?", w.DistributorID).
			Update("balance", gorm.Expr("balance + ?", w.Amount))
		global.DB.Create(&model.CommissionLog{
			DistributorID: w.DistributorID, Amount: w.Amount, Type: "adjust", Remark: "提现拒绝退回",
		})
		global.DB.Model(&w).Updates(map[string]interface{}{"status": 2, "reject_reason": reason, "audit_at": &now})
	}
	return nil
}
...
func GetCommissionLogs(distributorID uint, page, pageSize int) (int64, []model.CommissionLog, error) {
	var total int64
	db := global.DB.Model(&model.CommissionLog{}).Where("distributor_id = ?", distributorID)
	db.Count(&total)
