...
func BuyOrderInit(userID uint, cartIDs []uint, buyType string) (*BuyInitResp, error) {
	...
	if addr.ID > 0 { resp.Address = &addr }
	...
	// 计算总价
	for _, g := range goods { resp.Total += g.Price * int64(g.Quantity) }
	...
	switch siteType {
	case "1": resp.SiteModel = 1
	case "2": resp.SiteModel = 2
	case "3": resp.SiteModel = 3
	default: resp.SiteModel = 0
	}
	return resp, nil
}
...
func BuyTypeGoodsList(userID uint, ids []uint, buyType string) ([]BuyGoodsItem, error) {
	...
	case "cart":
		...
		for i, c := range carts {
			...
			if c.Goods != nil { items[i].Title = c.Goods.Title; items[i].Image = c.Goods.MainImage }
			if c.SKU != nil { items[i].SKUName = c.SKU.Name; items[i].Price = c.SKU.Price }
		}
		return items, nil
	case "goods":
		if len(ids) < 2 { return nil, errors.New("参数错误") }
...
func BuyGoodsCheck(goodsID, skuID uint, quantity int) error {
	var goods model.Goods
	if err := global.DB.First(&goods, goodsID).Error; err != nil { return errors.New("商品不存在") }
	if goods.Status != 1 { return errors.New("商品已下架") }
	var sku model.GoodsSKU
	if err := global.DB.First(&sku, skuID).Error; err != nil { return errors.New("规格不存在") }
	if sku.Stock < quantity { return fmt.Errorf("库存不足(剩余%d)", sku.Stock) }
	return nil
}

// BuyDataStorage 购买数据临时存储（Redis）
func BuyDataStorage(userID uint, data interface{}) error {
	if global.RDB == nil { return nil }
...
func BuyDataRead(userID uint) (map[string]interface{}, error) {
	if global.RDB == nil { return nil, nil }
