...
func TestCreateOrder_Integration(t *testing.T) {
	...
	app.Must().DB.First(&updatedSKU, sku.ID)
	...
	// Verify cart cleared
	var cartCount int64
	app.Must().DB.Model(&model.Cart{}).Where("user_id = 1").Count(&cartCount)
	if cartCount != 0 {
