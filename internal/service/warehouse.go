...
func CreateWarehouse(req *WarehouseReq) (*model.Warehouse, error) {
	...
	return &w, app.Must().DB.Create(&w).Error
}

func GetWarehouseList() ([]model.Warehouse, error) {
	var list []model.Warehouse
	return list, app.Must().DB.Order("level DESC, id ASC").Find(&list).Error
}

func UpdateWarehouse(id uint, req *WarehouseReq) error {
	return app.Must().DB.Model(&model.Warehouse{}).Where("id = ?", id).Updates(map[string]interface{}{
...
func DeleteWarehouse(id uint) error {
	var count int64
	app.Must().DB.Model(&model.WarehouseGoods{}).Where("warehouse_id = ?", id).Count(&count)
	if count > 0 {
		return errors.New("仓库下有商品，无法删除")
	}
	return app.Must().DB.Delete(&model.Warehouse{}, id).Error
}

func UpdateWarehouseStatus(id uint, status int8) error {
	return app.Must().DB.Model(&model.Warehouse{}).Where("id = ?", id).Update("is_enable", status).Error
}

// 仓库商品管理
func WarehouseGoodsAdd(warehouseID, goodsID uint, inventory int) error {
	var wg model.WarehouseGoods
	app.Must().DB.Where("warehouse_id = ? AND goods_id = ?", warehouseID, goodsID).Find(&wg)
	if wg.ID > 0 {
		return app.Must().DB.Model(&wg).Update("inventory", inventory).Error
	}
	return app.Must().DB.Create(&model.WarehouseGoods{WarehouseID: warehouseID, GoodsID: goodsID, Inventory: inventory, IsEnable: 1}).Error
}

func WarehouseGoodsList(warehouseID uint) ([]model.WarehouseGoods, error) {
	var list []model.WarehouseGoods
	return list, app.Must().DB.Where("warehouse_id = ?", warehouseID).Find(&list).Error
}
...
func WarehouseGoodsSpecSave(warehouseID, goodsID, skuID uint, inventory int, specValues string) error {
	...
	app.Must().DB.Where("warehouse_id = ? AND goods_id = ? AND sku_id = ?", warehouseID, goodsID, skuID).Find(&ws)
	if ws.ID > 0 {
		return app.Must().DB.Model(&ws).Update("inventory", inventory).Error
	}
	return app.Must().DB.Create(&model.WarehouseGoodsSpec{WarehouseID: warehouseID, GoodsID: goodsID, SKUID: skuID, Inventory: inventory, SpecValues: specValues}).Error
}

// WarehouseGoodsInventory 查询某商品在所有仓库的总库存
func WarehouseGoodsInventory(goodsID uint) int {
	var total int64
	app.Must().DB.Model(&model.WarehouseGoods{}).Where("goods_id = ? AND is_enable = 1", goodsID).
