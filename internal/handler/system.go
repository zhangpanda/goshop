...
func CreateBrand(c *gin.Context) {
	var req struct { Name string `json:"name" binding:"required"`; Logo string `json:"logo"`; Desc string `json:"desc"`; Sort int `json:"sort"` }
	...
	if err != nil { response.Fail(c, http.StatusInternalServerError, err.Error()); return }
	response.OK(c, b)
}
...
func GetArticleDetail(c *gin.Context) {
	...
	if err != nil { response.Fail(c, http.StatusNotFound, "文章不存在"); return }
	response.OK(c, a)
}
...
func SaveGoodsParams(c *gin.Context) {
	id, _ := strconv.ParseUint(c.Param("id"), 10, 64)
	var req struct { Params []service.ParamsConfigItem `json:"params"` }
	if err := c.ShouldBindJSON(&req); err != nil { response.Fail(c, http.StatusBadRequest, err.Error()); return }
...
func CreateSlideHandler(c *gin.Context) {
	var s model.Slide; c.ShouldBindJSON(&s); service.CreateSlide(&s); response.OK(c, s)
}
func GetNavigationList(c *gin.Context) { list, _ := service.NavigationList(c.Query("type")); response.OK(c, list) }
func CreateNavigationHandler(c *gin.Context) {
	var n model.Navigation; c.ShouldBindJSON(&n); service.CreateNavigation(&n); response.OK(c, n)
}
func GetLinkList(c *gin.Context) { list, _ := service.LinkList(); response.OK(c, list) }
func CreateLinkHandler(c *gin.Context) {
	var l model.Link; c.ShouldBindJSON(&l); service.CreateLink(&l); response.OK(c, l)
}

// ---- 支付方式 ----
func GetPaymentList(c *gin.Context) { list, _ := service.PaymentList(); response.OK(c, list) }
func CreatePaymentHandler(c *gin.Context) {
	var p model.Payment; c.ShouldBindJSON(&p); service.CreatePayment(&p); response.OK(c, p)
}
...
func GetAttachmentCategoryList(c *gin.Context) {
	list, _ := service.AttachmentCategoryList(); response.OK(c, list)
}
func CreateAttachmentCategoryHandler(c *gin.Context) {
	var req struct { Name string `json:"name" binding:"required"` }
	if err := c.ShouldBindJSON(&req); err != nil { response.Fail(c, http.StatusBadRequest, err.Error()); return }
	service.CreateAttachmentCategory(req.Name); response.OK(c, nil)
}
