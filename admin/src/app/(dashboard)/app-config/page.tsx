...
export default function Page() {
  ...
  useEffect(() => { api.get<CI[]>('/admin/config?group=app').then(r => setItems(Array.isArray(r)?r:[])).catch(()=>setItems([])) }, [])
  useEffect(() => {
  ...
  return (<><Typography.Title level={4}>基础配置</Typography.Title><Card><Form form={form} layout="vertical">
    ...
    {items.length>0 && <Form.Item><Button type="primary" size="large" onClick={async()=>{
      ...
      await api.post('/admin/config',{group:'app',configs}); message.success('保存成功')
