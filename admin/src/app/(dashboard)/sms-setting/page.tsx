...
export default function Page() {
  ...
  useEffect(() => { api.get<CI[]>('/admin/config?group=sms').then(r => setItems(Array.isArray(r)?r:[])).catch(()=>setItems([])) }, [])
  useEffect(() => {
  ...
  return (<><Typography.Title level={4}>短信设置</Typography.Title><Card><Form form={form} layout="vertical">
    ...
    {items.length>0 && <Form.Item><Button type="primary" size="large" onClick={async()=>{
      ...
      await api.post('/admin/config',{group:'sms',configs}); message.success('保存成功')
