...
import { Tabs, Form, Input, InputNumber, Switch, Select, Button, Typography, message, Card } from 'antd'
import { api } from '@/lib/api'
import ImageUpload from '@/components/ImageUpload'

const GROUPS = [
  { key: 'base', label: '基础配置' }, { key: 'site', label: '站点设置' },
  { key: 'seo', label: 'SEO配置' }, { key: 'order', label: '订单配置' },
  { key: 'email', label: '邮件配置' }, { key: 'sms', label: '短信配置' },
  { key: 'attachment', label: '附件配置' }, { key: 'cache', label: '缓存配置' },
  { key: 'admin', label: '后台配置' }, { key: 'app', label: '客户端配置' },
  { key: 'weixin', label: '微信配置' }, { key: 'alipay', label: '支付宝配置' },
]
...
const SEL: Record<string,{value:string;label:string}[]> = {
  common_sms_type:[{value:'aliyun',label:'阿里云'},{value:'tencent',label:'腾讯云'}],
  common_email_smtp_type:[{value:'ssl',label:'SSL'},{value:'tls',label:'TLS'},{value:'',label:'无'}],
  home_seo_url_model:[{value:'0',label:'兼容模式'},{value:'1',label:'非兼容模式'}],
  common_timezone:[{value:'Asia/Shanghai',label:'亚洲/上海'},{value:'Asia/Tokyo',label:'亚洲/东京'},{value:'America/New_York',label:'美国/纽约'}],
}
...
function field(k: string): ReactNode {
  ...
  if (SEL[k]) return <Select options={SEL[k]} />
...
export default function ConfigPage() {
  ...
  useEffect(() => {
    api.get<CI[]>(`/admin/config?group=${group}`).then(r => setItems(Array.isArray(r) ? r : [])).catch(() => setItems([]))
  }, [group])

  useEffect(() => {
    const v: Record<string, unknown> = {}
    items.forEach(i => {
      if (SW.has(i.key)) v[i.key] = i.value === '1'
      else if (NUM.has(i.key)) v[i.key] = i.value ? Number(i.value) : undefined
      else v[i.key] = i.value
    })
  ...
  const save = async () => {
    ...
    for (const [k, v] of Object.entries(raw)) {
      if (SW.has(k)) configs[k] = v ? '1' : '0'
      else configs[k] = v == null ? '' : String(v)
    }
    await api.post('/admin/config', { group, configs })
  ...
  return (
    <>
      ...
      <Tabs activeKey={group} onChange={g => { setGroup(g); form.resetFields() }}
        items={GROUPS.map(g => ({ key: g.key, label: g.label }))} />
      <Card>
        <Form form={form} layout="vertical">
          {items.map(i => (
            <Form.Item key={i.key} name={i.key}
              label={<>{i.desc || i.key} <span style={{ color: '#aaa', fontSize: 11 }}>({i.key})</span></>}
              valuePropName={SW.has(i.key) ? 'checked' : 'value'}>
              {field(i.key)}
            </Form.Item>
          ))}
          {items.length > 0 && <Form.Item><Button type="primary" onClick={save} size="large">保存配置</Button></Form.Item>}
          {items.length === 0 && <Typography.Text type="secondary">暂无配置项</Typography.Text>}
        </Form>
      </Card>
    </>
  )
