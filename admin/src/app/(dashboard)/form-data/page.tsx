...
export default function FormDataPage() {
  ...
  return (<><Typography.Title level={4}>表单数据</Typography.Title>
    ...
    <Table dataSource={list} rowKey="id" pagination={{current:page,total,pageSize:20,onChange:p=>load(p),showTotal:t=>`共 ${t} 条`}} columns={[
      ...
      {title:'用户ID',dataIndex:'user_id',width:80},{title:'数据',dataIndex:'data',ellipsis:true,render:(v:string)=>{try{const d=JSON.parse(v||'{}');const keys=Object.keys(d);return keys.slice(0,3).map(k=>`${k}:${typeof d[k]==='object'?'...':d[k]}`).join(' | ')+(keys.length>3?' ...':'')}catch{return v}}},
    ...
    <DetailDrawer open={!!detail} onClose={()=>setDetail(null)} title={`表单数据 #${detail?.id}`} items={detail?[
      {label:'表单ID',value:detail.form_id},{label:'用户ID',value:detail.user_id},
